...
**`decode_b64_image(image: str, *, filename=None) -> ipy.File`**
Decodes a base64 image string (strips `data:image/...` prefix if present) into an `ipy.File` backed by `io.BytesIO`. Pair with `ipy.EmbedAttachment("attachment://{filename}")` to embed in-memory images.
